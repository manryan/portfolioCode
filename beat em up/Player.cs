...
public class Player : Entity
{
    ...
    public bool canMove()
    {
        ...
        if (!crouch)
        {
            if (!floored && movement != Vector2.zero && !blocking && !playerHit && !rolling && !attacking && (Mathf.Abs(movement.x) > 0.3f || Mathf.Abs(movement.y) > 0.3f))
            {
                return true;
            }
            else
        ...
        {
            if (!floored && movement != Vector2.zero && !blocking && !playerHit && !rolling && !attacking && (Mathf.Abs(movement.x) > 0.3f || Mathf.Abs(movement.y) > 0.3f) && canWalk)
            {
                return true;
            }
            else
