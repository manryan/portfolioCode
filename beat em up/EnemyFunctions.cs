...
public class EnemyFunctions : MonoBehaviour
{
    ...
    public IEnumerator stepaside()
    {
        enemy.newspot = false;

        yield return new WaitUntil(() => enemy.anim.GetCurrentAnimatorStateInfo(0).IsName("Idle") || enemy.anim.GetCurrentAnimatorStateInfo(0).IsName("Walk"));
        ...
        enemy.spot = transform.position + ((transform.position - enemy.p.transform.position) * 0.5f);
       ...
       enemy.sideStepSpot = enemy.spot;
        ...
        if (enemy.sidestepcols.Length < 1)
        {
            ...
            enemy.sidestepray = new Ray(new Vector3(transform.position.x, 0.8f, Mathf.Clamp( transform.position.z, -2f,2f)), new Vector3(transform.position.x, 0.8f, Mathf.Clamp(transform.position.z, -2f, 2f)) - new Vector3(enemy.p.transform.position.x, 0.8f, enemy.p.transform.position.z));
    ...
    public bool tryeastorwest()
    {
        //we're left
        if(transform.position.x<enemy.player.position.x)
        {
            if (shorterfunctionsidestep(-1))
                return true;
        }
        else
        {
    ...
    void tryOtherSpots()
    {
        //   go east or west before south or north?

        if (tryeastorwest())
            return;

        //south
        if (transform.position.z < enemy.p.transform.position.z)
        {
            //we want to go south
            goSouthOrNorth(-1);
        }
    ...
    bool shorterfunctionsidestep(int dir)
    {
        enemy.spot = transform.position + (Vector3.right* dir);
    ...
    void keepFindingNewSideSpot(float northorsouth)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                enemy.StartCoroutine(enemy.tryToDoLiveMovement());
 ...
 /*   public IEnumerator continueToPlayer()
    ...
    void movens(int ns)
    {
       noneFound = true;
        ...
        for (float i = 0f; i < distToThatWall; i += 0.2f)
        {
            ...
            if (ns == 1)
            {
                safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x + 2.2f, enemy.player.transform.position.x + 2.2f, enemy.cam.transform.position.x + 3.5f), transform.position.y, transform.position.z + i);
            }
            else
            {
                safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x + 2.2f, enemy.player.transform.position.x + 2.2f, enemy.cam.transform.position.x + 3.5f), transform.position.y, transform.position.z - i);
    ...
    void movensWest(int ns)
    {
        ...
        for (float i = 0f; i < distToThatWall; i += 0.2f)
        {
            ...
            if (ns == 1)
            {
                safeLocation = new Vector3(Mathf.Clamp(enemy.p.transform.position.x - 2.2f, enemy.cam.transform.position.x - 3.5f, enemy.p.transform.position.x - 2.2f), transform.position.y, transform.position.z + i);
            }
            else
            {
                safeLocation = new Vector3(Mathf.Clamp(enemy.p.transform.position.x - 2.2f, enemy.cam.transform.position.x - 3.5f, enemy.p.transform.position.x - 2.2f), transform.position.y, transform.position.z - i);
         ...
         //   Debug.Log("left of us");
            ...
            if (!barelyAnyDist())
            {
                ...
                if (enemyinsafelocationcheck())
                {
                    ...
                    if (GameManager.instance.canMoveCamera || safeLocation.x < enemy.cam.transform.position.x + 3.5f)
                    {
                        //set safelocation                
                        //     nav.velocity = Vector3.zero;
                        safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x + 2.2f, enemy.player.transform.position.x + 2.2f, enemy.cam.transform.position.x + 3.5f), transform.position.y, transform.position.z);
                        enemy.cango = true;
                        return;
                    }
                    else
        ...
        else //right of us
        {
            ...
            if (!barelyAnyDist())
            {
                ...
                if (enemyinsafelocationcheck())
                {
                    ...
                    //         if (GameManager.instance.canMoveCamera || transform.position.x < cam.transform.position.x+ 5f)
                    if (safeLocation.x > enemy.cam.transform.position.x - 3.5f)
                    {
                        //set safelocation                
                        //     nav.velocity = Vector3.zero;
                        safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x - 2.2f, enemy.cam.transform.position.x - 3.5f, enemy.player.transform.position.x - 2.2f), transform.position.y, transform.position.z);
                        enemy.cango = true;
                        return;
                    }
                    else
        ...
        for (float i = 0f; i < distToThatWall; i += 0.2f)
        {
            ...
            if (leftOrRight == -1)
            {
                if (southDist > northDist) //south wall is further
                {
                    ...
                    safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x - 2.2f - i, enemy.cam.transform.position.x - 3.5f, enemy.player.transform.position.x - 2.2f - i), 1.32f, Mathf.Clamp(transform.position.z + i, transform.position.z + i, 2f));
                }
                else
                {
                    safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x - 2.2f - i, enemy.cam.transform.position.x - 3.5f, enemy.player.transform.position.x - 2.2f - i), 1.32f, Mathf.Clamp(transform.position.z - i, -2f, transform.position.z - i));
                }
            ...
            else
            {
                if (southDist > northDist)//south wall is further
                {
                    safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x + 2.2f + i, enemy.player.transform.position.x + 2.2f + i, enemy.cam.transform.position.x + 3.5f), 1.32f, Mathf.Clamp(transform.position.z + i, transform.position.z + i, 2f));
                }
                else
                {
                    safeLocation = new Vector3(Mathf.Clamp(enemy.player.transform.position.x + 2.2f + i, enemy.player.transform.position.x + 2.2f + i, enemy.cam.transform.position.x + 3.5f), 1.32f, Mathf.Clamp(transform.position.z - i, -2f, transform.position.z - i));
                }
                //   enemyInSafeLocation = Physics.OverlapSphere(new Vector3(safeLocation.x, 0.4f, transform.position.z + i), 0.2f, rollh.enemyBox);
            }
  ...
  float wallS;
   ...
   public bool alternativeSideSpot()
    ...
    bool up()
    {
        for (float i = 0.75f; i < 2; i += 0.5f)
        {
            enemy.livespotref = new Vector3(enemy.player.position.x, 0, enemy.player.position.z + i);

            enemy.spotcols = Physics.OverlapSphere(new Vector3(enemy.livespotref.x, 0.7f, enemy.livespotref.z), 0.3f, enemy.objectsnplayer);
    ...
    {
     playerCols = Physics.OverlapSphere(transform.position, 3f, enemy.playerMask);

        if (playerCols.Length > 0)
        {
            for (int i = 0; i < playerCols.Length; i++)
            {
                if (Vector3.Dot(playerCols[i].transform.root.transform.position - transform.position, new Vector3(enemy.wanderspot.x, 1.32f,enemy.wanderspot.z) - new Vector3(transform.position.x, 1.32f, transform.position.z)) > 0.75f)
                {
                    return true;
                }
            }
    ...
    public int returnUsableSpots()
    {
        ...
        // jumpAngle = transform.position - p.transform.position;
        //  lastAngle = 500;

        foreach (Vector3 vecs in  enemy.jumpRollPositions)
        {
            empty = true;

            //   compareAngle = transform.position - (transform.position + new Vector3(transform.GetChild(0).localScale.x * vecs.x, vecs.y, vecs.z));

            spotcols = Physics.OverlapSphere(new Vector3(transform.position.x, 0, transform.position.z) + new Vector3(enemy.leftorRight() * vecs.x, 0.8f, vecs.z), 0.1f, enemy.invisipne);

            enemy.wanderray = new Ray(new Vector3(transform.position.x, 0.8f, Mathf.Clamp( transform.position.z, -1.8f, 1.8f)), ((new Vector3(transform.position.x, 0.8f, Mathf.Clamp(transform.position.z, -1.8f, 1.8f)) + new Vector3(enemy.leftorRight() * vecs.x, 0f, vecs.z)) - new Vector3(transform.position.x, 0.8f, Mathf.Clamp(transform.position.z, -1.8f, 1.8f))));

            usablehits = Physics.RaycastAll(enemy.wanderray.origin, enemy.wanderray.direction, 1f, enemy.invisipne);
        ...
        if (usableSpots.Count == 2 && usableSpots.Contains(enemy.jumpRollPositions[3]) && usableSpots.Contains(enemy.jumpRollPositions[4]))
        {
            if (enemy.p.transform.position.z <= transform.position.z)
            {
        ...
        {
            foreach (Vector3 vecs in usableSpots.ToArray())
            {
                if (enemy.p.transform.position.z < transform.position.z)
                {
                    //left
            ...
            {
                vectTouse = (transform.position + (usableSpots[Random.Range(0, usableSpots.Count)] * enemy.leftorRight())) - transform.position;
            }
            else
            {
                //roll away?
